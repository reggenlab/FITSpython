...
class FITSPhase2:
    def __init__(self, dataName, topk, colWise, name2save):
        self.startProcess( dataName, topk, colWise, name2save)

    def startProcess(self, dataName, topk, colWise, name2save):
        dataX = np.loadtxt(dataName, delimiter=",")
        dataX = np.transpose(dataX)
        average = dataX.mean(axis=1)
        dataX = dataX/average[:, np.newaxis]+0.000001
